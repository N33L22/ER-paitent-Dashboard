...
if 'exec_realtime_enabled' not in st.session_state:
    st.session_state.exec_realtime_enabled = False

if 'exec_update_interval' not in st.session_state:
    st.session_state.exec_update_interval = 10

if 'exec_last_update' not in st.session_state:
    st.session_state.exec_last_update = datetime.now()

# Real-time controls in sidebar
st.sidebar.markdown("---")
st.sidebar.markdown("### 🔴 Real-Time Streaming")

exec_interval = st.sidebar.slider("Update Interval", 5, 60, st.session_state.exec_update_interval, format="%ds", key="exec_interval")
st.session_state.exec_update_interval = exec_interval

rt_col1, rt_col2 = st.sidebar.columns(2)
with rt_col1:
    if st.button("▶️ Start", use_container_width=True, key="exec_start"):
        st.session_state.exec_realtime_enabled = True
        st.session_state.exec_last_update = datetime.now()
with rt_col2:
    if st.button("⏸️ Stop", use_container_width=True, key="exec_stop"):
        st.session_state.exec_realtime_enabled = False

if st.session_state.exec_realtime_enabled:
    time_since = (datetime.now() - st.session_state.exec_last_update).total_seconds()
    st.sidebar.markdown(f'''
    <div style="background: rgba(56, 161, 105, 0.2); border-radius: 8px; padding: 0.5rem; margin: 0.5rem 0;">
        <p style="color: #38a169; font-size: 0.85rem; margin: 0;">🔴 <b>LIVE</b> - Next update: {max(0, exec_interval - int(time_since))}s</p>
    </div>
    ''', unsafe_allow_html=True)
    st.sidebar.progress(min(1.0, time_since / exec_interval))
...
if st.session_state.exec_realtime_enabled:
    time_since_update = (datetime.now() - st.session_state.exec_last_update).total_seconds()
    
    if time_since_update >= st.session_state.exec_update_interval:
        st.session_state.exec_last_update = datetime.now()
        time.sleep(0.1)
        st.rerun()
    else:
