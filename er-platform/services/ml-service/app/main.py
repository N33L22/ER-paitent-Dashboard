...
from .model_evaluation import ModelEvaluator, TaskType
...
async def lifespan(app: FastAPI):
    ...
    try:
        model_store.model_evaluator = ModelEvaluator()
...
async def get_model_status():
    ...
    return {
        ...
        "model_evaluator": {
            "initialized": model_store.model_evaluator is not None
        },
...
async def evaluate_classification_model(request: EvaluationRequest):
    """
    ...
    """
    if model_store.model_evaluator is None:
    ...
    metrics = model_store.model_evaluator.evaluate_classification(
        y_true=y_true,
        y_pred=y_pred,
        y_prob=y_prob,
    ...
    model_store.last_evaluation = result
    return result


@app.post("/evaluate/regression")
async def evaluate_regression_model(request: EvaluationRequest):
    """
    ...
    metrics = model_store.model_evaluator.evaluate_regression(
        y_true=y_true,
        y_pred=y_pred,
        model_name=request.model_name
    )
    ...
    groups = {}
    if request.acuity:
        groups['acuity'] = np.array(request.acuity)
    if request.age_group:
        groups['age_group'] = np.array(request.age_group)
    
    error_analysis = model_store.model_evaluator.analyze_errors(
        y_true=y_true,
        y_pred=y_pred,
...
async def evaluate_los_model(request: EvaluationRequest):
    """
    Specialized LOS prediction evaluation with clinical metrics.
    
    Returns:
    - Regression metrics (MAE, RMSE, R²)
    - Clinical accuracy (within 30/60/120 min)
    - Category accuracy (short/medium/long stay)
    - Stratified analysis by acuity and age
    """
    if model_store.model_evaluator is None:
        raise HTTPException(status_code=503, detail="Model evaluator not initialized")
    ...
    y_true = np.array(request.y_true)
    y_pred = np.array(request.y_pred)
    acuity = np.array(request.acuity) if request.acuity else None
    age_group = np.array(request.age_group) if request.age_group else None
    
    result = model_store.model_evaluator.evaluate_los_prediction(
        y_true=y_true,
        y_pred=y_pred,
        acuity=acuity,
        age_group=age_group
...
@app.post("/evaluate/threshold-analysis")
async def analyze_thresholds(request: EvaluationRequest):
    """
    Analyze model performance at different classification thresholds.
    
    Helps optimize precision-recall tradeoff.
    """
    if model_store.model_evaluator is None:
        raise HTTPException(status_code=503, detail="Model evaluator not initialized")
    
    if request.y_prob is None:
        raise HTTPException(status_code=400, detail="y_prob required for threshold analysis")
    
    y_true = np.array(request.y_true)
    y_prob = np.array(request.y_prob)
    
    result = model_store.model_evaluator.analyze_threshold(
        y_true=y_true,
        y_prob=y_prob
    )
...
async def get_last_evaluation():
    ...
    if model_store.last_evaluation is None:
        raise HTTPException(status_code=404, detail="No evaluation performed yet")
    return model_store.last_evaluation
...
async def compare_models(
    ...
    Returns ranked comparison with best model.
    """
    if model_store.model_evaluator is None:
        raise HTTPException(status_code=503, detail="Model evaluator not initialized")
    
    y_true_arr = np.array(y_true)
    preds = {k: np.array(v) for k, v in predictions.items()}
    probs = {k: np.array(v) for k, v in probabilities.items()} if probabilities else None
    
    task = TaskType.REGRESSION if task_type == "regression" else TaskType.BINARY_CLASSIFICATION
    
    result = model_store.model_evaluator.compare_models(
        y_true=y_true_arr,
        predictions=preds,
        probabilities=probs,
        task_type=task
